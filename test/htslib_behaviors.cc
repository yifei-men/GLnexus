...
TEST_CASE("htslib hfile_mem VCF/BCF serialization") {
    ...
    *(size_t**)(pfn+4+sizeof(void**)) = &bufsz;
    ...
    //
    // Yikes! We may need a significant htslib patch to make it truly able to
    // read & write "uncompressed BCF"

    UPD(vcfFile, bcfmem, bcf_open(pfn, "wu"), [](vcfFile* f) { bcf_close(f); });

    //REQUIRE(bcf_hdr_write(bcfmem, hdr) == 0);
    for (const auto& rec : records) {
        REQUIRE(bcf_write(bcfmem, hdr, rec.get()) == 0);
    }
    size_t memlen = htell(bcfmem->fp.hfile);
    REQUIRE(hflush(bcfmem->fp.hfile) == 0);
    ...
    bufsz = memlen;
    up_bcfmem.reset(bcf_open(pfn, "r"));
    bcfmem = up_bcfmem.get();

    for (const auto& rec : records) {
        REQUIRE(vcf_read(bcfmem, hdr, vt.get()) == 0);
